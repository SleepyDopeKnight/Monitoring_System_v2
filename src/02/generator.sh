...
function generate {
    ...
    if [[ ${#folders} -lt 5 ]]; then #if folder name < 5 symbols
        for (( k=${#folders}; k<5; k++)); do
            folders+="$(echo $new_folders_name)"
        done
    fi

    if [[ ${#file_name} -lt 5 ]]; then #if folder name < 5 symbols
    ...
    for (( i=1; i<=$count_for_cycle; i++)); do
        folder_generator="$(compgen -d / | shuf -n1 )" #randoming directory for generating
        count_of_files="$(shuf -i 1-50 -n1)" #randoming number of files
        if [[ $folder_generator == '/bin' || $folder_generator == '/sbin' ]]; then #skipping this folders
            count_for_cyclet=$(($count_for_cycle + 1))
            continue
        fi
        sudo mkdir -p "$folder_generator"/"$folders"_"$date"
        for (( j=1; j<=$count_of_files; j++)); do
            echo "Directories were created: $i/${count_for_cycle} and files were created $j/${count_of_files}"
            free_space=$(df -h / -BM | grep '/' | awk -F"M" {'print $3'})
            if [[ $free_space -le 1024 ]]; then
                echo "1GB of memory left on the drive"
                exit 1
            fi
            sudo fallocate -l $file_size "$folder_generator"/"$folders"_"$date"/"$file_name"_"$date"."$file_extension" #set file size
            ...
            file_name+="$(echo $new_file_name)"
            while [ $count -eq $count_of_files ]; do #set file name for new directory
